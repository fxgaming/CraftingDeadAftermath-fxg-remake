...
import by.fxg.craftingdead.damage.DamageSourceBleeding;
import by.fxg.craftingdead.damage.DamageSourceBullet;
import by.fxg.craftingdead.damage.DamageSourceBulletHeadshot;
import by.fxg.craftingdead.damage.DamageSourceCD;
import by.fxg.craftingdead.damage.DamageSourceRBInfection;
...
import by.fxg.craftingdead.item.ItemClothingArmor;
import by.fxg.craftingdead.item.ItemClothingJuggernaut;
import by.fxg.craftingdead.item.ItemHatHelmet;
...
import by.fxg.craftingdead.item.potion.PotionManager;
import by.fxg.craftingdead.network.CDAPacketACLInCombat;
import by.fxg.craftingdead.network.CDAPacketHandcuffInteract;
...
public class CommonEvents {
	...
	public void onPlayerInteractWithEntity(EntityInteractEvent event) {
		if (event.entityPlayer != null) {
			PlayerData data = CDReloaded.instance.playerDataHandler().getPlayerData(event.entityPlayer);
	...
	public void onItemLeftClick(AttackEntityEvent event) {
		if (event.entityPlayer != null) {
			PlayerData playerdata = CDReloaded.instance.playerDataHandler().getPlayerData(event.entityPlayer);
			if (playerdata.isHandCuffed) {
				event.setCanceled(true);
			}
		}
	...
	public void onFlamethrowerDamages(EventFlamethrowerSetFire event) {
		if (event.entityHit instanceof EntityPlayer) {
			...
			PlayerData data = CDReloaded.instance.playerDataHandler().getPlayerData(playerHit);
	...
	public void onEntityKilled(LivingDeathEvent event) {
		...
		if (event.entityLiving instanceof EntityPlayer) {
			...
			if (!player.worldObj.isRemote) {
				data = CDReloaded.instance.playerDataHandler().getPlayerData(player);
				data.onPlayerDeath();
			}
			if (event.source.getSourceOfDamage() instanceof EntityPlayer) {
				EntityPlayer playerKilling = (EntityPlayer) event.source.getSourceOfDamage();
				if (!playerKilling.worldObj.isRemote) {
					data = CDReloaded.instance.playerDataHandler().getPlayerData(playerKilling);
					++data.killedPlayers;
				}
			}
		}
...
//	}
	...
	public void onBulletCollision(EventBulletCollision event) {
		if (!event.getWorldObject().isRemote) {
			if (event.getCollisionType() == EnumBulletCollision.ENTITY) {
				...
				PlayerData hitData = CDReloaded.instance.playerDataHandler().getPlayerData(playerHit);
		...
		if (e.item.getEntityItem() != null && e.entityLiving instanceof EntityPlayer) {
			if (e.item.getEntityItem().itemID == ItemManager.silver.itemID || e.item.getEntityItem().itemID == ItemManager.gold.itemID) {
				if (!e.entity.worldObj.isRemote) {
					PlayerData data = CDReloaded.instance.playerDataHandler().getPlayerData((EntityPlayer) e.entityLiving);
	...
	public void applyBulletCollisionEntity(EntityPlayer par1, ItemStack par2, EntityLivingBase par3, boolean par4, Vec3 hitVec, World par6) {
		...
		if (!MinecraftForge.EVENT_BUS.post(eventBulletCollision)) {
			...
			if (par3 instanceof EntityPlayer && !par4 && !par3.isDead && par3.getHealth() > 0.0F) {
				...
				PlayerData hitData = CDReloaded.instance.playerDataHandler().getPlayerData(player);
	...
	public void applyBulletCollisionBlock(EntityPlayer par1, ItemStack itemstack, World par2, double par3, double par4, double par5, int par6, int par7, Vec3 hitVec) {
		if (par1 instanceof EntityPlayerMP) {
			...
			PlayerData data = CDReloaded.instance.playerDataHandler().getPlayerData((EntityPlayer) playerMP);
