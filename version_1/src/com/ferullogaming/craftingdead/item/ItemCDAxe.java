...
import com.ferullogaming.craftingdead.events.EventBaseMaterialHarvested;
...
public class ItemCDAxe extends ItemMeleeWeapon {
   ...
   public float axeStrength = 1.2F;

   public ItemCDAxe(int par1, int par2, int par3) {
      super(par1, par2, par3);
   }

   public ItemCDAxe setAxeDropRate(int par1) {
      this.dropRate = par1;
      return this;
   }

   public ItemCDAxe setAxeStrength(float par1) {
      this.axeStrength = par1;
      return this;
   }
   ...
   public boolean onBlockStartBreak(ItemStack itemstack, int X, int Y, int Z, EntityPlayer player) {
      ...
      if (player.capabilities.isCreativeMode) {
         return false;
      } else if (blockid != 0 && blockid == Block.wood.blockID) {
         Random rand = new Random();
         if (!world.isRemote) {
            int dropAmount = rand.nextInt(this.dropRate) + 1;
            ItemStack dropStack = new ItemStack(ItemManager.logSplit.itemID, dropAmount, 0);
   ...
   public float getStrVsBlock(ItemStack par1ItemStack, Block par2Block) {
      return this.isBlockIDAllowed(par2Block.blockID) ? this.axeStrength : 1.0F;
   }

   public boolean isBlockIDAllowed(int par1) {
      int[] var2 = ItemManager.destroyableAxeIDs;
   ...
   public void addInformation(ItemStack par1ItemStack, EntityPlayer par2EntityPlayer, List par3List, boolean par4) {
      par3List.add("Собирайте дерево вырубая деревья!");
