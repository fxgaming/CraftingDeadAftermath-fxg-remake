...
import com.ferullogaming.craftingdead.item.ItemHat;
...
public class CDAPacketGrenadeFlash extends CDAPacket {
   public static Packet buildPacket(double distance, int id) {
      ...
      try {
         data.write(NetworkManager.getIDFromPacket(CDAPacketGrenadeFlash.class));
   ...
   public void execute(DataInputStream stream, EntityPlayer player, Object[] extradata, Side side) {
      try {
         ...
         PlayerData data = CDAftermath.instance.playerDataHandler().getPlayerData(player.username);
         if (this.isInFieldOfVision(player.worldObj.getEntityByID(id), player)) {
            data.flashTime += (26.0D - distance) * 0.7D;
         } else {
            data.flashTime = 2.0999999999999996D;
         }

         if (data.flashTime > 10.0D) {
            data.flashTime = 10.0D;
         }

         if (data.getCDInventory().getStack("hat") != null && ((ItemHat)data.getCDInventory().getStack("hat").getItem()).canReduceFlahes()) {
            data.flashTime /= 2.0D;
         }
