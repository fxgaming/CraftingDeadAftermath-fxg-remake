...
import com.ferullogaming.craftingdead.client.gui.GuiCDMainMenu;
...
import com.ferullogaming.craftingdead.client.util.TickChecker;
...
import com.ferullogaming.craftingdead.network.CDAPacketACLUnpaused;
import com.ferullogaming.craftingdead.network.CDAPacketBulletCollision;
import com.ferullogaming.craftingdead.network.CDAPacketOpenGUI;
import com.ferullogaming.craftingdead.network.CDAPacketPlayerDataToServer;
import com.ferullogaming.craftingdead.network.CDAPacketSwitchItem;
...
public class ClientTickHandler implements ITickHandler {
	...
	private void onPlayerClientTick(EntityPlayer entityPlayer) {
		if (entityPlayer != null && !entityPlayer.isDead) {
			CDAftermath.instance.getItemManager().getPotionManager().onClientPlayerTick(entityPlayer);
		}
	}
	...
	private void onClientTick(Minecraft mc) {
		...
		PlayerData data;
		if (mc.theWorld != null) {
			CDAftermath.instance.getSoundHandler().onIngameUpdate();
			data = CDAftermath.instance.playerDataHandler().getClientPlayerData();
		...
		if (mc.thePlayer != null) {
			data = CDAftermath.instance.playerDataHandler().getPlayerData(mc.thePlayer.username);
	...
	private void onTickInGame(Minecraft mc) {
		if (mc.theWorld != null && mc.thePlayer != null) {
			PlayerData data = CDAftermath.instance.playerDataHandler().getPlayerData(mc.thePlayer.username);
	...
	public void spawnBullet(EntityPlayer par1, ItemGun par2) throws Exception {
		if (par1 != null && par1.worldObj != null && par1.worldObj.isRemote) {
			PlayerData dataShooter = CDAftermath.instance.playerDataHandler().getClientPlayerData();
					...
					if (entity != par1) {
						if (entity instanceof EntityLivingBase) {
							...
							if (entity instanceof EntityPlayer) {
								...
								PlayerData dataHit = CDAftermath.instance.playerDataHandler().getPlayerData(playerHit.username);
	...
	public MovingObjectPosition rayTrace(EntityLivingBase living, double par1, float par3) {
		...
		if (living instanceof EntityPlayer) {
			...
			if (CDAftermath.instance.playerDataHandler().getPlayerData(player).isAiming) {
				flag = false;
				if (player.getCurrentEquippedItem() != null && player.getCurrentEquippedItem().getItem() instanceof ItemGun) {
					ItemGun gun = (ItemGun) player.getCurrentEquippedItem().getItem();
	...
	public static ClientTickHandler getInstance() {
		return CDAftermath.instance.getClientManager().getClientTickHandler();
	}
}
